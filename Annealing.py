...
def Monte_Carlo_Sampling(lattice,basis,J_Beta,Sample_Size):
    ...
    for i in range(Sample_Size):
        ...
        _State_Energy = State_Energy(lattice,basis,basis.index(state))
        Prob_Beta = NN_State_Probability(J_Beta,_State_Energy)
        basis.flip(state,index)
        _State_Energy = State_Energy(lattice,basis,basis.index(state))
        Prob_Alpha = NN_State_Probability(J_Beta,_State_Energy)
        probability_append = min(1,Prob_Alpha / Prob_Beta)
        if np.random.rand() <= probability_append:
            Energy_Array = np.append(Energy_Array,_State_Energy)
...
Monte_Carlo_Sampling(lattice,basis,1,1000000)
